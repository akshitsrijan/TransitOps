# TransitOps — RAG Backend

Retrieval-Augmented Generation service for the TransitOps Transport Management System.

**Scope disclaimer:** this repo owns the RAG component only — document ingestion,
embedding, retrieval, and grounded LLM summarization. Frontend and the CRUD backend
are owned by other teams and are out of scope here. Integration point with them is
MySQL (operational data) and, eventually, a shared API contract.
...
If no chunk survives, the LLM must respond with an explicit "insufficient context"
message rather than generate an answer. The LLM's role is strictly to summarize
what was retrieved — it does not answer from parametric knowledge. This must be
enforced in the prompt template *and* validated by checking that every response
carries at least one retrieved-source citation.
...
│   │   ├── retriever.py       # ChromaDB query + threshold filtering
│   │   └── llm.py             # grounded summarization, citation enforcement
│   ├── db/
...
cp .env.example .env
# fill in MySQL credentials, LLM_API_KEY, etc.
```
...
| `LLM_*` | Summarization model config — temperature fixed low/zero by default to reduce generative drift |
...
Scaffolding only. Ingestion pipeline, retriever, LLM summarization layer, and
API routes are not yet implemented — pending next steps.
